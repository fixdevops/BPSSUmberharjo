// ─── database.web.ts — localStorage + auto-sync ke Google Drive ──────────────
// localStorage = cache cepat di browser
// Google Drive = penyimpanan utama permanen
// Setiap operasi write otomatis push ke Drive (debounced 2 detik)

import { scheduleDriveSync } from "./driveStorage";
...
function lsGet<T>(key: string): T[] {
  try { return JSON.parse(localStorage.getItem("se2026_" + key) ?? "[]"); } catch { return []; }
}
function lsSet<T>(key: string, data: T[]): void {
  try { localStorage.setItem("se2026_" + key, JSON.stringify(data)); } catch (_) {}
}
function lsNextId(key: string): number {
  const cur = parseInt(localStorage.getItem("se2026_cnt_" + key) ?? "0", 10);
  const next = cur + 1;
  localStorage.setItem("se2026_cnt_" + key, String(next));
  return next;
}
function now(): string {
...
export async function insertSLS(data: { nama: string; kode: string; kecamatan?: string; kabupaten?: string; catatan?: string }): Promise<number> {
...
export async function getRTBySLS(slsId: number): Promise<RT[]> {
  ...
  return all.map((r) => {
    ...
    return { ...r, jumlah_bangunan: bIds.length, jumlah_kk: kk.filter((k) => bIds.includes(k.bangunan_id)).length };
...
export async function getRTById(id: number): Promise<RT | null> {
  ...
  return { ...r, jumlah_bangunan: bangunan.length, jumlah_kk: lsGet<KK>("kk").filter((k) => bIds.includes(k.bangunan_id)).length };
}
export async function insertRT(data: { sls_id: number; nama_rt: string; nama_rw?: string; ketua_rt?: string; catatan?: string }): Promise<number> {
...
export async function insertKK(data: { bangunan_id: number; nomor_urut?: string; nama_kk: string; nama_kepala?: string; jumlah_anggota?: number; catatan?: string }): Promise<number> {
  const list  = lsGet<KK>("kk");
  const id    = lsNextId("kk");
  const nomor = data.nomor_urut?.trim() || await nextNomorKK(data.bangunan_id);
  list.push({ id, bangunan_id: data.bangunan_id, nomor_urut: nomor, nama_kk: data.nama_kk, nama_kepala: data.nama_kepala ?? null, jumlah_anggota: data.jumlah_anggota ?? 1, catatan: data.catatan ?? null, synced: 0, created_at: now() });
  lsSet("kk", list);
  scheduleDriveSync();
  return id;
}
...
export async function insertFoto(data: { bangunan_id: number; kk_id?: number; uri: string; keterangan?: string }): Promise<number> {
  const list = lsGet<Foto>("foto");
  const id   = lsNextId("foto");
  list.push({ id, bangunan_id: data.bangunan_id, kk_id: data.kk_id ?? null, uri: data.uri, cloud_url: null, keterangan: data.keterangan ?? null, synced: 0, created_at: now() });
  lsSet("foto", list);
  scheduleDriveSync();
  return id;
}
