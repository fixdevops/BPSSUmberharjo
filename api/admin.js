...
export default async function handler(req, res) {
  ...
  if (req.method === "POST" && action === "mint") {
    ...
    const keyType = req.body?.type || "app"; // "app" | "lapangan"
...
<body>
  ...
  <div class="card">
    ...
    <div class="mint-form">
      ...
      <div class="input-group" style="max-width:200px">
        <label for="typeSelect">Tipe Kunci</label>
        <select id="typeSelect" style="padding:10px 14px;background:#0f0f1a;border:1px solid #2a2a4a;border-radius:8px;color:#e0e0e0;font-size:14px;outline:none;cursor:pointer">
          <option value="app">🔑 App (Kalkulator saja)</option>
          <option value="lapangan">📋 Lapangan (Kalkulator + Data Lapangan + Drive)</option>
        </select>
      </div>
    ...
    <div class="result-box" id="resultBox">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        <span id="resultTypeBadge" style="padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;background:#1a3f6e;color:#4ade80"></span>
      </div>
  ...
  <script>
    ...
    async function mintKey() {
      ...
      const type  = document.getElementById('typeSelect').value || 'app';
      btn.disabled = true;
      ...
      try {
        ...
        if (data.success) {
          ...
          badge.textContent = type === 'lapangan' ? '📋 Lapangan (Data Lapangan + Drive)' : '🔑 App (Kalkulator)';
          badge.style.background = type === 'lapangan' ? '#1a5c3a' : '#1a3f6e';
          badge.style.color      = type === 'lapangan' ? '#4ade80' : '#60a5fa';
